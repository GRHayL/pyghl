...
def cons_to_x_W_guess(
    ...
    *,
    ...
    width_tiny = float(bundle["y_stats"].get("width_tiny", 1.0e-12))
    width = 1.0 + float(q)
    if (not np.isfinite(width)) or (width <= width_tiny):
        raise ValueError(
            f"Invalid Palenzuela width: 1+q={width:.6e} <= width_tiny={width_tiny:.6e}"
    ...
    yhat = predict_numpy_like(bundle["model"], x_in, bundle["ft_stats"], bundle["x_stats"], bundle["y_stats"])
    return (
        float(yhat[0, 0].detach().cpu().item()),
        float(yhat[0, 1].detach().cpu().item()),
    )
